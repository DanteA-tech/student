...
<body>
    <h1>Search for Clash Royale Cards</h1>
    <input type="text" id="searchBox" placeholder="Enter card name">
    <button onclick="searchCard()">Search</button>
    <div id="cardData">
        <!-- Card data will be displayed here -->
    </div>
    <script>
        // Sample data format - replace with your actual card data
        const cardData = [
        {
  "items": [
    {
    ...
    },
    {
      ...
      "rarity": "epic"
        ...
        function searchCard() {
            ...
            const result = cardData.find(card => card.name.toLowerCase() === searchText);
            const displayArea = document.getElementById('cardData');
            displayArea.innerHTML = ''; // Clear previous results
            if (result) {
                displayArea.innerHTML = `<div>
                    <h2>${result.name}</h2>
                    <p>ID: ${result.id}</p>
                    <p>Max Level: ${result.maxLevel}</p>
                    <p>Elixir Cost: ${result.elixirCost}</p>
                    <p>Rarity: ${result.rarity}</p>
                    <img src="${result.iconUrls.medium}" alt="${result.name}" />
                </div>`;
            } else {
                displayArea.innerHTML = '<p>Card not found.</p>';
            }
        }
    </script>
</body>
