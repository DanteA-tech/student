...
<head>
    ...
    <style>
        ...
        /* New styles for saving and displaying decks */
        #save-deck-container input[type="text"] {
            margin-right: 10px;
            padding: 5px;
            font-size: 16px;
        }
        #save-deck-container button, #saved-decks button {
            padding: 5px 10px;
            font-size: 16px;
            cursor: pointer;
        }
...
<body>
    ...
    <div id="cards"></div>
   <!-- New HTML for saving and displaying decks -->
    <div id="save-deck-container" style="margin-top: 20px; text-align: center; display: none;">
        <input type="text" id="deck-name" placeholder="Enter deck name" />
        <button onclick="saveDeck()">Save Deck</button>
    </div>
    <div id="saved-decks" style="margin-top: 20px;"></div>
    <script>
        <script>
        var cardData = {
    "items": [
      ...
      "rarity": "common"
       ...
       function displaySavedDecks() {
            ...
            Object.keys(savedDecks).forEach(deckName => {
                ...
                savedDecksContainer.appendChild(button);
            });
        }
        function displayDeck(deckName) {
            var deckIds = savedDecks[deckName];
            var deckBuilder = document.getElementById('deck-builder');
            deckBuilder.innerHTML = '';
            deck = deckIds;
            deckIds.forEach(cardId => {
                var card = cardData.items.find(item => item.id === cardId);
                if (card) { // Ensure the card exists in the data
                    addToDeck(card, true); // Add to deck without checking the deck size
                }
            });
            updateAverageElixir();
            document.getElementById('save-deck-container').style.display = 'none';
        }
    </script>
</body>
</html>
