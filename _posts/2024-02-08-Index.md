...
<body>
    ...
    <!-- Save Deck Button -->
    <button id="save-deck" style="display:none;" onclick="saveDeck()">Save Deck</button>
    <div id="cards"></div>
    <!-- Section for displaying saved decks -->
    <h2>Saved Decks</h2>
    ...
    },
    {
      ...
      "rarity": "common"
       ...
       function sortCards(criteria, order = 'asc') {
        ...
        var savedDecks = []; // Array to store saved decks
        function updateSaveDeckButton() {
            document.getElementById('save-deck').style.display = deck.length === 8 ? 'block' : 'none';
        }
        function saveDeck() {
            var title = prompt("Enter a title for your deck:");
            var description = prompt("Enter a description for your deck:");
            if (title && description) {
                var deckDetails = {
                    title: title,
                    description: description,
                    cards: deck.slice() // Copy the current deck
                };
                savedDecks.push(deckDetails);
                displaySavedDecks();
                deck = []; // Clear the current deck
                document.getElementById('deck-builder').innerHTML = ''; // Clear the displayed deck
                updateAverageElixir();
        ...
        function displaySavedDecks() {
            ...
            savedDecksContainer.innerHTML = ''; // Clear previous saved decks
            savedDecks.forEach(function(deck, index) {
                var deckElem = document.createElement('div');
                deckElem.innerHTML = `<h3>${deck.title}</h3><p>${deck.description}</p>`;
                savedDecksContainer.appendChild(deckElem);
            });
        }

        // Modify the addToDeck function to include updateSaveDeckButton call
        // Modify the removeCardFromDeck function to include updateSaveDeckButton call

        // Call displayCards at the end of your script to initialize the card display
        displayCards();
    </script>
</body>
</html>
